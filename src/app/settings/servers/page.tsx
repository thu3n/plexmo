...
import { Plus, Trash2, Edit2, CheckCircle, AlertCircle, RefreshCw, X } from "lucide-react";
...
export default function ServersSettingsPage() {
    const { t } = useLanguage();
    const { data, isLoading, error, mutate } = useSWR<{ servers: PublicServer[] }>("/api/servers", fetchJson);
                ...
                {isLoading ? (
                    ...
                    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        {data?.servers.map((server) => (
                            <SettingsCard key={server.id} className="group relative flex flex-col justify-between min-h-[200px] border-l-4" style={{ borderLeftColor: server.color || '#eab308' }}>
                                <div>
...
function ServerModal({ isOpen, onClose, server, onSuccess }: { isOpen: boolean, onClose: () => void, server: PublicServer | null, onSuccess: () => void }) {
    ...
    const [formData, setFormData] = useState({
        ...
        token: server?.maskedToken || "", // Note: logic might needed to handle masked/raw token on submit
        color: server?.color || ""
    });
    ...
    // ... Reuse logic from original file for handlers ...
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Handle masked token: if it has ellipsis and unchanged, don't send it or handle in backend. 
            // Here we assume if it's new it's raw, if existing and unchanged we might send it but backend ignores if masked.
            // Actually, simplest is to only send token if it doesn't contain ellipsis OR if it's a new server

            const payload: any = { ...formData };
            if (payload.token.includes("…") && server) {
                delete payload.token; // Don't update token if masked
            }
                ...
                {discoveryMode === "auto" && !server && discoveryToken && (
                    <div className="mb-6 space-y-2">
                        ...
                        >
                            ...
                            {flatConnections.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                        </select>
                    </div>
                )}
