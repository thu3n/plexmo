...
export default function LibraryPage({ params }: { params: Promise<{ key: string }> }) {
    ...
    const { data, error, isLoading } = useSWR<{ items: any[], totalCount: number, server?: { baseUrl: string, token: string } }>(
        serverId ? `/api/libraries/${key}/items?server=${serverId}&page=${page}&pageSize=${pageSize}` : null,
        fetchJson,
                ...
                <div className="rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl space-y-6">
                    ...
                    {isLoading ? (
                        ...
                        <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                            {data?.items.map((item) => (
                                <div key={item.ratingKey} className="group relative flex flex-col gap-2">
                                    <div className="aspect-[2/3] overflow-hidden rounded-xl bg-slate-900 shadow-lg transition group-hover:scale-105 group-hover:shadow-2xl ring-1 ring-white/10">
                                    ...
                                    <div className="absolute inset-0 bg-black/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3 rounded-xl pointer-events-none">
