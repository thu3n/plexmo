...
// Helper for split title formatting (Dashboard Style)
export const getSplitDisplayTitle = (entry: HistoryEntry, details: PlexSession | null, locale: string) => {
    if (!details) return { mainTitle: entry.title, subTitle: "" };

    let mainTitle = entry.title;
    let subTitle = "";
    const isTV = !!details.grandparentTitle;

    if (isTV) {
        mainTitle = details.grandparentTitle!;
        ...
        if (parentIndex !== undefined && index !== undefined) {
            ...
            const episodeTitle = details.originalTitle || details.title;
