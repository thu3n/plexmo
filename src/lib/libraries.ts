...
export const getLibraryItemsPaginated = async (libraryKey: string, serverId: string, page: number = 1, pageSize: number = 50) => {
    ...
    const items = db.prepare(`
        ...
        ORDER BY title ASC
        LIMIT ? OFFSET ?
    `).all(libraryKey, serverId, pageSize, offset);

    const total = db.prepare(`
        SELECT COUNT(*) as count FROM library_items 
        WHERE libraryKey = ? AND serverId = ?
    `).get(libraryKey, serverId) as { count: number };
